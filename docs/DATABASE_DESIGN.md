...
SkillSwap's only persistence before this project was Firebase Realtime Database — no
SQL anywhere in the repository (this was flagged as an
...
that choice was made), this project adds a real, minimal, standalone relational
...
- **No index on `description`.** Never filtered or sorted on. Indexing a column
  nothing queries is exactly the "fake complexity" this project was asked to avoid.
